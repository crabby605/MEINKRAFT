...
const skyHeight = 10; // Number of tiles for the sky
...
const world = Array(worldHeight).fill().map((_, y) => {
    return Array(worldWidth).fill().map((_, x) => {
        ...
        if (y >= worldHeight - 3) return 0; // Dirt layer above bedrock
        return 1; // Grass layer on top
    });
});
...
function calculateOffsets() {
    ...
    const verticalOffset = (canvas.height - (worldHeight * tileSize + skyHeight * tileSize)) / 2;
            ...
            if (sprite.complete) {
                if (breakQueue.has(`${x},${y}`)) {
                    ...
                    if (Date.now() - breakTime >= breakDuration) {
                        ...
                        ctx.globalAlpha = 0.5;
                        ctx.drawImage(sprite, x * tileSize + horizontalOffset, y * tileSize + skyHeight * tileSize + verticalOffset, tileSize, tileSize);
                        ctx.globalAlpha = 1.0;
                        continue;
                    }
                } else {
...
function isMouseOverBlock(x, y) {
    ...
    const blockY = y * tileSize + skyHeight * tileSize + verticalOffset;
